...
class GeneralFuntions:
    ...
    def get_opening_hours_list_or_error(openingHours_data):
        opening_hours_list = []
        for opening_hours in openingHours_data:
            opening_hours_id = opening_hours.pop('id', None)
            if opening_hours_id is not None:
                openingHours_instance_or_error = GeneralFuntions.get_instance_or_error(
                ...
                opening_hours_serializer = OpeningHoursSerializer(
                    openingHours_instance_or_error, data=opening_hours, partial=True)
